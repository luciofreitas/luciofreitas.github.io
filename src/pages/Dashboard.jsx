...
import { TrendingUp, AlertTriangle, DollarSign, ShoppingBag, Users } from 'lucide-react'
...
export default function Dashboard() {
  const { vendas, pedidos, clientes, estoque } = useApp()
  ...
  const vendasSemana = useMemo(() => vendas.filter(v => new Date(v.data) >= inicioSemana), [vendas])
  ...
  const topProdutos = useMemo(() => {
    ...
    return Object.entries(contagem)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 5)
      .map(([nome, qty]) => ({ nome: nome.length > 18 ? nome.slice(0, 16) + '…' : nome, qty }))
  }, [vendasSemana])
    ...
    <div className="space-y-4 lg:space-y-6">
      ...
      <div className="grid lg:grid-cols-2 gap-4 lg:gap-6">
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 lg:p-5">
          <h3 className="font-semibold text-gray-700 mb-3 text-sm">Receita — Últimos 7 dias</h3>
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 lg:p-5">
          <h3 className="font-semibold text-gray-700 mb-3 text-sm">Mais vendidos (semana)</h3>
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={topProdutos} layout="vertical">
              <XAxis type="number" tick={{ fontSize: 11 }} />
              <YAxis dataKey="nome" type="category" tick={{ fontSize: 10 }} width={110} />
      ...
      {/* Próximas entregas */}
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 lg:p-5">
        <h3 className="font-semibold text-gray-700 mb-3 text-sm">Próximas Entregas</h3>
          ...
          {pedidosPendentes.slice(0, 5).map(p => (
            <div key={p.id} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0 gap-3">
              <div className="min-w-0">
                ...
                <p className="text-xs text-gray-400 truncate">{p.itens.map(i => i.nome).join(', ')}</p>
              </div>
