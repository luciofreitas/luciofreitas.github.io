...
import { Plus, Minus, Trash2, ShoppingCart, Check, Search, X } from 'lucide-react'
...
export default function PDV() {
  ...
  function finalizar() {
    ...
    if (formaPag === 'Dinheiro' && valorRecebido) setTroco(Number(valorRecebido) - total)
    setConcluida(true)
  ...
  if (concluida) {
    return (
      <div className="flex flex-col items-center justify-center min-h-80 gap-6 px-4">
        ...
        <button onClick={novaVenda} className="bg-brownie-600 text-white px-8 py-3 rounded-lg font-semibold hover:bg-brownie-700 transition-colors">
          Nova Venda
        </button>
      </div>
    )
  ...
  const CarrinhoPanel = () => (
    <div className="bg-white border border-gray-100 rounded-xl shadow-sm flex flex-col h-full">
      ...
      <div className="p-4 border-t border-gray-100 space-y-3">
        ...
        {formaPag === 'Dinheiro' && (
          <input type="number" placeholder="Valor recebido (R$)" value={valorRecebido}
        ...
        <button onClick={finalizar} disabled={carrinho.length === 0}
  ...
  return (
    <>
      {/* Desktop layout: produtos + carrinho lado a lado */}
      <div className="hidden lg:flex gap-4" style={{ minHeight: 'calc(100vh - 120px)' }}>
        {/* Produtos */}
        <div className="flex-1 flex flex-col gap-4 min-w-0">
          ...
          <div className="flex gap-2 flex-wrap">
            {categorias.map(cat => (
              <button key={cat} onClick={() => setCategoriaFiltro(cat)}
                className={`px-3 py-1 rounded-full text-sm font-medium transition-colors ${categoriaFiltro === cat ? 'bg-brownie-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
                {cat}
              </button>
            ))}
          </div>
          <div className="grid grid-cols-2 xl:grid-cols-3 gap-3 overflow-y-auto flex-1">
            {produtosFiltrados.map(prod => (
              <button key={prod.id} onClick={() => addAoCarrinho(prod)}
      ...
      {/* Mobile layout: produtos em lista + botão flutuante do carrinho */}
      <div className="lg:hidden space-y-3 pb-24">
        ...
        <div className="flex gap-2 overflow-x-auto pb-1">
          {categorias.map(cat => (
            <button key={cat} onClick={() => setCategoriaFiltro(cat)}
              className={`px-3 py-1.5 rounded-full text-xs font-medium transition-colors flex-shrink-0 ${categoriaFiltro === cat ? 'bg-brownie-600 text-white' : 'bg-gray-100 text-gray-600'}`}>
              {cat}
            </button>
          ))}
        </div>
        <div className="grid grid-cols-2 gap-3">
          {produtosFiltrados.map(prod => (
            <button key={prod.id} onClick={() => addAoCarrinho(prod)}
              className="bg-white border border-gray-100 rounded-xl p-3 text-left hover:border-brownie-300 active:scale-95 transition-all">
              <p className="font-semibold text-gray-800 text-sm leading-tight">{prod.nome}</p>
              <p className="text-xs text-gray-400 mt-1">{prod.categoria}</p>
              <p className="text-brownie-600 font-bold mt-2 text-sm">R$ {prod.preco.toFixed(2)}</p>
            </button>
          ))}
        </div>
      </div>
      ...
      {!verCarrinho && (
        <button
          ...
          className="lg:hidden fixed bottom-6 right-6 bg-brownie-600 text-white rounded-2xl px-5 py-3.5 shadow-2xl flex items-center gap-3 font-semibold z-30"
        >
          <ShoppingCart size={20} />
          <span>Carrinho</span>
          {totalItens > 0 && (
            <span className="bg-white text-brownie-700 text-xs font-bold rounded-full w-5 h-5 flex items-center justify-center">
              {totalItens}
            </span>
          )}
          {total > 0 && <span className="text-sm opacity-90">· R$ {total.toFixed(2)}</span>}
        </button>
      )}
      ...
      {verCarrinho && (
        <div className="lg:hidden fixed inset-0 z-50 flex flex-col">
          <div className="flex-1 bg-black/40" onClick={() => setVerCarrinho(false)} />
